...
namespace FPSCounter
{
    ...
    public class FrameCounter : BasePlugin
    {
        ...
        public override void Load()
        {
            ...
            _position.SettingChanged += (sender, args) => FrameCounterComponent.UpdateLooks();
            _counterColor.SettingChanged += (sender, args) => FrameCounterComponent.UpdateLooks();
            ...
            {
                FrameCounterComponent.UpdateLooks();
                ComponentInstance.SetCapturingEnabled(_shown.Value);
            };
            _showPluginStats.SettingChanged += (sender, args) => ComponentInstance.SetCapturingEnabled(_shown.Value);

            ComponentInstance = AddComponent<FrameCounterComponent>();
            ...
            ComponentInstance.Invoke(nameof(ComponentInstance.OnEnable), 0.5f);
        }

        public sealed class FrameCounterComponent : MonoBehaviour
        {
            private static readonly Il2CppSystem.Type[] _helperTypes;
            ...
            internal void SetCapturingEnabled(bool enableCapturing)
            {
                if (!enableCapturing)
                {
                    PluginCounter.Stop();
                ...
                {
                    if (_helpers[0] == null) _helpers[0] = gameObject.AddComponent(_helperTypes[0]).Cast<MonoBehaviour>();
                    if (_helpers[1] == null) _helpers[1] = gameObject.AddComponent(_helperTypes[1]).Cast<MonoBehaviour>();
